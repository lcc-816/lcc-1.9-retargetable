...
int main(argc, argv) char *argv[]; {
	...
	if (glevel || xref) {
		...
		src.y = lineno;
		if (glevel > 2 || xref)
			stabend(&src, symroot, (Coordinate **)litoa(append((Generic)0, loci), 0),
				(Symbol *)litoa(append((Generic)0, tables), 0),
				symbols ? (Symbol *)litoa(symbols, 0) : 0);
		else
			stabend(&src, 0, 0, 0, 0);
	}
#ifdef progend
...
static int doargs(argc, argv) char *argv[]; {
	...
	for (i = j = 1; i < argc; i++)
		...
		|| strncmp(argv[i], "-a", 2) == 0)
			...
			density = (float)strtod(&argv[i][2], 0);
...
static Type ftype(rty, ty) Type rty, ty; {
	...
	return func(rty, (Type *)litoa(list, (Generic *)alloc((length(list) + 1)*sizeof (Type))));
}
...
static void bbcall(Symbol, Coordinate *, Tree *);
static void bbentry(Symbol, Symbol);
static void bbexit(Symbol, Symbol, Tree);
static int bbfile(char *);
static void bbfunc(Symbol, Symbol);
static void bbincr(Symbol, Coordinate *, Tree *);
static void bbvars(Symbol);
...
static void tracecall(Symbol, Symbol);
static void tracefinis(Symbol);
static void tracereturn(Symbol, Symbol, Tree);
static void tracevalue(Tree, int);
