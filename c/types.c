...
static Field check(Type, Type, Field, int);
static Type funcl(Type, List);
static Field isfield(char *, Field);
static Type tynode(int, Type, int, int, Generic);
...
static Type funcl(ty, list) Type ty; List list; {
	return func(ty, (Type *)litoa(list, (Generic *)alloc((length(list) + 1)*sizeof (Type))));
}
