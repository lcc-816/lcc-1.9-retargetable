...
static void checklab(Symbol, Generic);
static void checkparam(Symbol, Generic);
static void checkref(Symbol, Generic);
...
static Type dclr(Type, char **, int);
static Type dclr1(char **, int);
static void decl(Symbol (*)(int, char *, Type, Coordinate *), int);
static void doglobal(Symbol, Generic);
static void doextern(Symbol, Generic);
static Type enumdecl(void);
static void fields(Type);
static void funcdecl(int, char *, Type, Coordinate);
static Type newstyle(Type, Symbol[], Symbol[]);
static void oldparam(Symbol, Generic);
static Type oldstyle(char *, Type, Symbol[], Symbol[]);
static Type *parameters(int);
static Type structdcl(int);
static Type tnode(int, Type);
static Type type(int, int *);
...
void compound(loop, swp, lev) struct swtch *swp; {
	...
	cp->u.block.locals = (Symbol *)litoa(registers, (Generic *)talloc((nregs + length(autos) + 1)*sizeof(Symbol)));
	litoa(autos, (Generic *)&cp->u.block.locals[nregs]);
...
Symbol (*dcl)(int, char *, Type, Coordinate *); {
	int sclass;
	char *id = 0;
	Type ty, ty1;
	Coordinate pt;
...
static Type enumdecl() {
	...
	if (t == '{') {
		...
		ty->u.sym->u.idlist = (Symbol *)litoa(idlist, (Generic *)alloc((length(idlist) + 1)*sizeof(Symbol)));
		ty->u.sym->defined = 1;
...
static void fillcallees(Symbol, Generic);
...
static void funcdecl(sclass, id, ty, pt) char *id; Type ty; Coordinate pt; {
	...
	caller = (Symbol *)litoa(callees, (Generic *)0);
...
static Type *parameters(lev) {
	...
	|| t == ID && tsym && tsym->sclass == TYPEDEF) {
		...
		proto = (Type *)litoa(list, (Generic *)alloc((n + 1)*sizeof (Type)));
	} else if (t == ID) {
		for (;;) {
			if (t != ID) {
